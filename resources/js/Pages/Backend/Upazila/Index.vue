...
const applyFilter = () => {
    router.get(route('backend.upazila.index'), filters.value, { preserveState: true, only: ['datas'], });
};
const divisionFilters = () => {
    filters.value.district='';
    router.visit(
        route('backend.upazila.index', filters.value),
        {
                ...
                <div class="grid w-full grid-cols-1 gap-2 md:grid-cols-5">
                    ...
                    <div class="">
                        <select id="division" v-model="filters.district"
                            class="block w-full p-2 text-sm rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600"
                            @change="applyFilter" v-if="districts?.length">
