...
const applyFilter = () => {
    router.get(route("backend.district.index"), filters.value, {
        preserveState: true,
    });
};
const divisionFilters = () => {
    router.visit(route("backend.district.index", filters.value), {
        only: ["datas"],
        preserveState: true,
    });
};
                ...
                <div class="grid w-full grid-cols-1 gap-2 md:grid-cols-5">
                    ...
                    <div class="">
                        <select
                            ...
                            @change="divisionFilters"
                            v-if="divisions?.length"
                        >
                            <option value="">-- Select A Division --</option>
                            <template
                                v-for="(divisionInfo, Index) in divisions"
                                :key="Index"
                            >
                                <option :value="divisionInfo.division_id">
